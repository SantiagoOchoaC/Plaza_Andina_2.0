...
function obtenerPedidosPorEstado($estado) {
    ...
    $sql = "SELECT 
            ...
            WHERE pg.estado_cocina = ? AND EXISTS (
                SELECT 1 FROM ticket_comida tb WHERE tb.pedido_id = pg.id
            )
...
function cambiarEstadoPedidoBarra() {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['pedido_id']) && isset($_POST['nuevo_estado'])) {
        ...
        try {
            $sql = "UPDATE pedido_general SET estado_cocina = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
...
function actualizarEstadoGeneral($pedido_id) {
    ...
    $sql = "SELECT estado_cocina, estado_cocina, estado_licor FROM pedido_general WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $pedido_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
function obtenerEstadisticas($estado) {
    ...
    $sql = "SELECT COUNT(*) as total FROM pedido_general WHERE estado_cocina = ? AND EXISTS (
                SELECT 1 FROM ticket_comida tb WHERE tb.pedido_id = pedido_general.id
            )";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $estado);
    $stmt->execute();
...
<body>
    ...
    function cambiarEstadoPedido(pedidoId, nuevoEstado) {
        ...
        fetch('?action=cambiar_estado_cocina', {
            method: 'POST',
            headers: {
